...
$$
\textsf{mod}(\mathcal{C}) = \sum_{k=1}^K (f_{k,k} - f^*_{k,k})^2\,,
$$
donde:

- $f_{k,\ell}$: fracción de enlaces en la red que conectan los vértices de $C_k$ con los vértices de $C_\ell$.
- $f^*_{k,\ell}$: valor esperado de $f_{k,\ell}$ bajo un **modelo de asignación** de aristas. 

Típicamente, $f^*_{k,\ell}=f_{k\bullet}f_{\bullet\ell}$ donde $f_{k\bullet}=\sum_{\ell=1}^Kf_{k,\ell}$ y $f_{\bullet\ell}=\sum_{k=1}^K f_{k,\ell}$ (para redes no dirigidas se tiene que $f_{k\bullet} = f_{\bullet\ell}$). Esta elección corresponde a un modelo diseñado para reproducir la distribución de grados de la red.
...
# grafos
par(mfrow = c(1,2))
igraph_options(vertex.size = 5, vertex.frame.color = "black")
set.seed(42)
l = layout_with_fr(karna)
