...
plot((0:max(d))[dd != 0], dd[dd != 0], log = "xy", pch = 16, col = "royalblue", xlab = "Log-grado", ylab = "Log-densidad", main = "Distribución de grado (log-log)")
```


Hay una porción importante de vértices de grado bajo, pero también hay una fracción no trivial de vértices con grados altos.
...
$$
f_d = a\,d^{-b}\,,\qquad a>0\,,\qquad b>0\,
$$
lo que en escala log corresponde a
$$
\log f_d = \log a - b\log d\,.
$$
$a$ se denomina constante de normalización y $b$ exponente de la ley de potencias.

Las redes que satisfacen este tipo de distribución del grado se denominan **libres de escala** (*scale free*).
...
# grado y GPV
plot(d, mnd, log = "xy", pch = 16, col = "yellow3", xlab = "Log-grado", ylab = "Log-grado promedio de los vecinos")
```
